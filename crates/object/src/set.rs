...
use erasure::bitrot::BitrotDetector;
use erasure::Erasure;
...
impl ErasureSet {
    ...
    pub fn build_version_header(
        ...
        header.meta_sys = metadata
            .iter()
            .map(|(k, v)| (k.clone(), v.as_bytes().to_vec()))
            .collect();
        header.meta_user = Vec::new();
...
mod tests {
    ...
    impl StorageAPI for MockDisk {
        async fn disk_info(&self) -> MinioResult<storage::DiskInfo> {
            unimplemented!()
        }
        ...
        async fn stat_file(&self, _: &str, _: &str) -> MinioResult<storage::FileStat> {
            unimplemented!()
        }
