//! Auto-generated msgpack serialization tests for listPathOptions type.
//!
//! Tests listPathOptions msgpack serialization/deserialization.


/// Tests listPathOptions Marshal/Unmarshal roundtrip.
#[test]
#[ignore]
fn test_marshal_unmarshal_list_path_options() {
    // TODO: implement when listPathOptions with msgp serialization is available
    /*
    let v = ListPathOptions::default();
    let bytes = v.marshal_msg(None).expect("marshal");
    let (v2, remaining) = ListPathOptions::unmarshal_msg(&bytes).expect("unmarshal");
    assert!(remaining.is_empty(), "bytes left after unmarshal");
    */
}

/// Tests listPathOptions Encode/Decode roundtrip.
#[test]
#[ignore]
fn test_encode_decode_list_path_options() {
    // TODO: implement when msgp encode/decode is available
}
