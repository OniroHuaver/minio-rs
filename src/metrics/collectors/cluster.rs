...
pub fn health_group(disk_infos: &[DiskInfo], total_disks: usize) -> MetricsGroup {
    let infos = vec![
        MetricInfo {
            name: "minio_cluster_health_status".into(),
            help: "Cluster health status (1 = online)".into(),
            metric_type: MetricType::Gauge,
            labels: vec![],
        },
        MetricInfo {
            name: "minio_cluster_disk_online".into(),
            help: "Number of online disks".into(),
            metric_type: MetricType::Gauge,
            labels: vec![],
        },
        MetricInfo {
            name: "minio_cluster_disk_total".into(),
    ...
    let status = prometheus::Gauge::with_opts(
        prometheus::Opts::new("minio_cluster_health_status", "Cluster health status (1 = online)"),
    )
    .unwrap();
    status.set(if online_count > 0 { 1.0 } else { 0.0 });

    let disk_online = prometheus::Gauge::with_opts(
        prometheus::Opts::new("minio_cluster_disk_online", "Number of online disks"),
    )
    .unwrap();
    disk_online.set(online_count as f64);

    let disk_total = prometheus::Gauge::with_opts(
        prometheus::Opts::new("minio_cluster_disk_total", "Total number of configured disks"),
    )
    .unwrap();
...
pub fn usage_group(_object_api: Arc<dyn crate::object::ObjectAPI>) -> MetricsGroup {
    let infos = vec![
        MetricInfo {
            name: "minio_cluster_usage_objects_total".into(),
            help: "Total number of objects in the cluster".into(),
            metric_type: MetricType::Gauge,
            labels: vec![],
        },
        MetricInfo {
            name: "minio_cluster_usage_total_bytes".into(),
    ...
    let objects = prometheus::Gauge::with_opts(
        prometheus::Opts::new(
            "minio_cluster_usage_objects_total",
...
pub fn erasure_set_group(disk_infos: &[DiskInfo], total_disks: usize) -> MetricsGroup {
    let infos = vec![
        MetricInfo {
            name: "minio_cluster_erasure_set_online".into(),
            help: "Erasure set online status (1 = healthy)".into(),
            metric_type: MetricType::Gauge,
            labels: vec![],
        },
        MetricInfo {
            name: "minio_cluster_erasure_set_drives".into(),
    ...
    let online = prometheus::Gauge::with_opts(
        prometheus::Opts::new(
            "minio_cluster_erasure_set_online",
    ...
    let drives = prometheus::Gauge::with_opts(
        prometheus::Opts::new(
            "minio_cluster_erasure_set_drives",
